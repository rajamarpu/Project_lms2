...
import { useCoursesQuery, useTeachersQuery } from '../../../api/reactQuery';
...
const DashboardTopPerformers = () => {
  const { data: coursesRes, isLoading: loading } = useCoursesQuery();
  ...
  const topCourses = courses
    ...
    .map((c, i) => ({
      ...
      gradient: 'linear-gradient(145deg, rgba(59,130,246,0.22) 0%, transparent 100%)',
      trend: '+14%',
      trendUp: true,
      enrollments: c._count?.enrollments || 0,
      enrollmentsDisplay: (c._count?.enrollments || 0).toLocaleString(),
      progress: Math.min(100, Math.round((c._count?.enrollments || 0) / 50)),
      sparkline: [10, 14, 18, 22, 28, 32, 40, 45].slice(0, 8),
    }));

  const items = topCourses.length ? topCourses : topPerformersSpotlight;

  if (loading) {
    return <div className="h-48 animate-pulse rounded-xl bg-[var(--admin-surface-raised)]" />;
  }
