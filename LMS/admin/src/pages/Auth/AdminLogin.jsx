...
const AdminLogin = () => {
  ...
  const handleGoogleLogin = async () => {
    // Google sign-in isn't wired to backend JWT in this app — prefer email login for admin.
    setError('Google login for admin is not supported. Please use email/password.');
  };
  ...
  const handleLogin = async (e) => {
    ...
    setError('');
    try {
      // Authenticate with backend to obtain JWT
      console.log('[UI] AdminLogin submitting', { email });
      const res = await adminLogin(email, password);
      console.log('[UI] AdminLogin response', res);
      if (res && res.token) {
        login({ id: res.user?.id || '', name: res.user?.name || email, email: res.user?.email || email }, 'admin', res.token);
      ...
      <div className="max-w-md w-full mx-4 glass-card glass-card-hover p-8 rounded-2xl relative z-10">
        ...
        <div className="text-center mb-6">
          ...
          <div className="bg-blue-500/10 border border-blue-500/30 p-2.5 rounded-lg text-xs text-blue-200 flex flex-col gap-1 items-center justify-center">
            <span className="font-semibold text-blue-400 uppercase tracking-wider text-[10px]">Use your registered admin account</span>
            <span>Sign in with the email and password you created during registration.</span>
          </div>
          <div className="mt-3 text-sm">
            <a href="/admin-register" className="text-accent-cyan hover:underline">Create an admin account</a>
          </div>
        </div>
        {error && (
        ...
        <p className="text-center text-xs text-gray-400 mt-2">
          Use Google or email/password to sign in as admin.
        </p>
      </div>
    </div>
  );
