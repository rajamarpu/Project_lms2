...
import { useStudentsQuery } from '../../../api/reactQuery';
...
const Students = () => {
  ...
  const { data: studentsRes, refetch: refetchStudents } = useStudentsQuery({ limit: 100 });
    ...
    <div className="admin-page space-y-6 md:space-y-8 animate-fade-in relative z-10 pb-16 min-h-full rounded-2xl p-4 md:p-6 -m-4 md:-m-6 border border-[var(--admin-border)] shadow-[var(--admin-shadow-card)] bg-[var(--admin-page-panel)]">
      ...
      <AddStudentDrawer
        ...
        onAdd={async (newStudent) => {
          if (studentToModify) {
            await studentsApi.updateStudent(studentToModify.id, buildStudentPayload(newStudent));
          } else {
            await studentsApi.createStudent(buildStudentPayload(newStudent));
          }
          await refetchStudents();
          handleCloseAddModal();
          setStudentToModify(null);
