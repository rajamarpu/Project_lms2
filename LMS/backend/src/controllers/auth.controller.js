...
exports.login = async (req, res, next) => {
  try {
    ...
    if (role && user.role !== role) {
      console.log('[AUTH] Role mismatch. Requested role:', role, 'User role:', user.role);
      return res.status(403).json({ success: false, error: 'Account does not have the required role. Please sign in with an admin account or register as an admin.' });
    }
    ...
    if (!isMatch) {
      console.log('[AUTH] Incorrect password for', normalizedEmail);
      return res.status(401).json({ success: false, error: 'Invalid credentials' });
    }
    ...
    const token = generateToken(user.id, user.role);
    console.log('[AUTH] JWT generated for', user.id, 'role:', user.role);
    ...
    console.log('[AUTH] Response:', { success: true, user: { id: user.id, email: user.email, role: user.role } });
