...
export const CourseCard = ({ course, index = 0 }: { course: Course; index?: number }) => {
  ...
  const progressVal = typeof course.progress === "number" ? course.progress : 0;
        ...
        <div className="flex flex-col flex-1 p-5 gap-3">
          ...
          <div className="grid grid-cols-2 gap-2 mt-auto pt-2">
            ...
            <Stat
              icon={TrendingUp}
              label="Progress"
              value={showProgress ? `${progressVal}%` : "Start"}
              accent="#06B6D4"
            />
          </div>
          ...
          <div className="mt-1">
            {showProgress ? (
              ...
              <p className="text-xs text-muted-foreground">Begin this course to track your progress.</p>
            )}
          </div>
        </div>
      </article>
