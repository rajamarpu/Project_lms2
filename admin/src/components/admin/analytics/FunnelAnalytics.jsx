...
// data: array of { stage, count, pct, color } from API
const FunnelAnalytics = ({ data, loading }) => {
      ...
      <div className="space-y-3">
        {loading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-10 rounded-xl animate-pulse" style={{ background: 'var(--admin-border)' }} />
          ))
        ) : funnelStages.length === 0 ? (
          <p className="text-sm admin-text-muted py-4">No funnel data available yet.</p>
        ) : (
              ...
              <div key={stage.stage}>
                <div className="flex items-center justify-between text-xs mb-1.5">
                  ...
                  <span className="admin-text-secondary tabular-nums">
                    {(stage.count ?? 0).toLocaleString()} · {stage.pct}%
                  </span>
                </div>
                <div
