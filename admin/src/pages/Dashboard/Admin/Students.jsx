  ...
  React.useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        const res = await fetch('http://localhost:5000/api/admin/users?role=user&limit=100', {
          headers: {
        ...
        if (data.success) {
          const mapped = data.data.map(u => ({
            ...
            mentorName: 'MS Dhoni',
            progress: 45,
            status: u.status === 'approved' ? 'Active' : u.status === 'suspended' ? 'Blocked' : 'Pending',
        ...
        onAdd={async (newStudent) => {
          if (studentToModify) {
            try {
              ...
              const res = await fetch(`http://localhost:5000/api/admin/users/${studentToModify.id}`, {
                method: 'PUT',
                headers: {
