...
const { findCourseById, createCourseWithLessons, generateCourseLessons } = require('../services/course.service');
...
exports.createCourse = async (req, res, next) => {
  try {
    ...
    const course = await createCourseWithLessons(courseData, generateAI);
...
exports.updateCourse = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== 'admin' && (req.user.role !== 'instructor' || course.instructorId !== req.user.id)) {
      return res.status(403).json({ success: false, error: 'Not authorized to update this course. Admin or course instructor only.' });
    }
...
exports.deleteCourse = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== 'admin' && (req.user.role !== 'instructor' || course.instructorId !== req.user.id)) {
      return res.status(403).json({ success: false, error: 'Not authorized to delete this course. Admin or course instructor only.' });
    }
...
exports.addLesson = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== 'admin' && (req.user.role !== 'instructor' || course.instructorId !== req.user.id)) {
      return res.status(403).json({ success: false, error: 'Not authorized to add lessons to this course. Admin or course instructor only.' });
    }
...
exports.deleteLesson = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== 'admin' && (req.user.role !== 'instructor' || course.instructorId !== req.user.id)) {
      return res.status(403).json({ success: false, error: 'Not authorized to delete lessons from this course. Admin or course instructor only.' });
    }
...
exports.generateLessonsAI = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== 'admin' && (req.user.role !== 'instructor' || course.instructorId !== req.user.id)) {
      return res.status(403).json({ success: false, error: 'Not authorized to generate lessons for this course. Admin or course instructor only.' });
    }

    const createdLessons = await generateCourseLessons(courseId, course.title, course.category, course.level);
