...
export function Features() {
  return (
    <div className="page-shell container py-12 md:py-16">
      ...
      <section className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {features.map(({ icon: Icon, title, text }) => (
          <article key={title} className="surface-card flex h-full flex-col rounded-[1.75rem] p-6">
            <div className="mb-5 flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Icon className="h-5 w-5" /></div>
            <h2 className="font-display text-lg font-semibold">{title}</h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">{text}</p>
          </article>
        ))}
      </section>
      <div className="mt-10 rounded-[1.75rem] border border-primary/20 bg-primary/5 p-6 text-center">
...
export function Support() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("The ticket could not be created. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="page-shell container py-12">
      <header className="mb-10 max-w-3xl">
        <p className="text-sm font-semibold text-primary">Help Center</p>
        <h1 className="mt-2 font-display text-3xl font-bold md:text-4xl">How can we help?</h1>
        <p className="mt-3 text-muted-foreground">Find an answer, review common issues, or raise a support request.</p>
      </header>
          ...
          <div className="mt-5 divide-y divide-border">
            {faqs.map(([question, answer], index) => (
              <div key={question} className="py-3">
                ...
                {openFaq === index && <p className="pt-3 text-sm leading-6 text-muted-foreground">{answer}</p>}
              </div>
            ))}
          </div>
        </section>

        <form onSubmit={submit} className="surface-card rounded-[1.75rem] p-6">
          ...
          <label className="mt-4 block text-sm font-medium">
            ...
            <textarea
              ...
              className="mt-2 min-h-32 w-full rounded-2xl border border-border bg-background p-3"
            />
          </label>
...
export function Notifications() {
  ...
  const markAll = async () => {
    await platformApi.readAllNotifications();
  ...
  return (
    <div className="page-shell container max-w-5xl py-12">
      <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-primary">Updates</p>
          <h1 className="mt-1 font-display text-3xl font-bold">Notification center</h1>
          <p className="mt-2 text-muted-foreground">Course, assignment, instructor, and certificate activity.</p>
        </div>
        <button type="button" className="btn-outline-teal !py-2" onClick={markAll}>Mark all read</button>
      </header>

      <section className="space-y-3" aria-busy={loading}>
        {loading ? (
          <p role="status" className="rounded-[1.75rem] border border-dashed border-border p-10 text-center text-muted-foreground">Loading notifications...</p>
        ) : items.length ? (
          items.map((item) => (
            <article key={item.id} className={`flex gap-4 rounded-[1.75rem] border p-5 ${item.readAt ? "border-border bg-card" : "border-primary/30 bg-primary/5"}`}>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <time className="text-xs text-muted-foreground" dateTime={item.createdAt}>{new Date(item.createdAt).toLocaleDateString()}</time>
                </div>
                <p className="mt-1 text-sm text-muted-foreground">{item.message}</p>
                {item.actionUrl && (
                  <button type="button" className="mt-3 text-sm font-semibold text-primary" onClick={() => open(item)}>
                    View details
                  </button>
                )}
              </div>
          ...
          <p className="rounded-[1.75rem] border border-dashed border-border p-10 text-center text-muted-foreground">You are all caught up.</p>
        )}
      </section>
    </div>
  );
...
export function Settings() {
  ...
  useEffect(() => {
    platformApi.preferences().then(({ data }) => setPrefs(data.data)).catch(() => toast.error("Preferences could not be loaded."));
  }, []);

  const options = useMemo(() => [
    ["courseNotifications", "Course updates"],
    ["deadlineReminders", "Assignments and assessments"],
    ["emailNotifications", "Email summaries"],
  ] as const, []);

  const save = async () => {
    try {
      const { data } = await platformApi.updatePreferences(prefs);
      setPrefs(data.data);
  ...
  return (
    <div className="page-shell container max-w-5xl py-12">
      <header className="mb-8">
        ...
        <p className="mt-2 text-muted-foreground">Control your theme, language, notifications, privacy, and account access.</p>
      </header>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <section className="surface-card rounded-[1.75rem] p-6">
          ...
          <select value={theme} onChange={(e) => setTheme(e.target.value as "light" | "dark" | "system")} className="mt-4 h-11 w-full rounded-2xl border border-border bg-background px-3">
        ...
        <section className="surface-card rounded-[1.75rem] p-6">
          ...
          <select value={prefs.language} onChange={(e) => setPrefs({ ...prefs, language: e.target.value })} className="mt-4 h-11 w-full rounded-2xl border border-border bg-background px-3">
            <option>English</option>
            <option>Hindi</option>
          </select>
        </section>

        <section className="surface-card rounded-[1.75rem] p-6 md:col-span-2">
          <div className="flex items-center gap-3">
          ...
          <div className="mt-4 divide-y divide-border">
            {options.map(([key, label]) => (
              <label key={key} className="flex min-h-14 items-center justify-between gap-3">
                <span className="text-sm">{label}</span>
                <input type="checkbox" checked={Boolean(prefs[key])} onChange={(e) => setPrefs({ ...prefs, [key]: e.target.checked })} className="h-5 w-5 accent-primary" />
              </label>
            ))}
          </div>
        </section>
        ...
        <section className="surface-card rounded-[1.75rem] p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Change your password and update personal information from your profile.</p>
          <Link to="/profile" className="mt-4 inline-block text-sm font-semibold text-primary">Open profile security</Link>
        </section>

        <section className="surface-card rounded-[1.75rem] p-6">
          <ShieldCheck className="mb-3 h-5 w-5 text-primary" />
          <h2 className="font-semibold">Privacy</h2>
          <p className="mt-2 text-sm text-muted-foreground">Your learning activity is used to calculate progress and recommendations.</p>
        </section>
      </div>

      <div className="mt-6 flex flex-wrap gap-3">
        <button type="button" onClick={save} className="btn-primary">Save settings</button>
        <button type="button" onClick={() => setPrefs(defaults)} className="btn-outline-teal">Reset</button>
      </div>
    </div>
  );
}
